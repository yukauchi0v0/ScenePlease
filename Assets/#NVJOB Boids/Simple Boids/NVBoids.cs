    ...
    void CreateFlock()
{
    ...
    float spacing = fragmentedFlock * 1.2f; // 控制群體之間間距
    Vector3 baseOffset = new Vector3(-spacing * (flockNum - 1) * 0.5f, 0, 0); // 從左到右展開
    ...
    for (int f = 0; f < flockNum; f++)
    {
        ...
        Vector3 offset = new Vector3(spacing * f, 0, Mathf.Sin(f * 1.5f) * spacing * 0.5f);
        flocksTransform[f].position = thisTransform.position + baseOffset + offset;
        flockPos[f] = offset;
